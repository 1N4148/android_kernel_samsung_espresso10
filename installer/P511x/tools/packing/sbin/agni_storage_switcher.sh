...
## AGNi pureCM KITKAT sdcard1<-->sdcard0 switcher v1.3
...
if [ -d /mnt/shell/emulated/0 ] && [ "`/sbin/busybox mount | grep fuse | grep sdcard1`" ];
	...
	/system/bin/ps | grep sdcard | awk '{print $2}' | xargs kill && chmod 0666 /system/bin/sdcard
	sync
	...
	#ln -s /mnt/shell/emulated/Android/obb /new-sdcard1-obb
