...
DEVICE_MODEL="P3110,P3113"
DEVICE_NAME="espressorf,espressowifi"
...
if ! [ "`grep init.agnimounts.rc /tmp/extracted/init.espresso.rc`" ];
	then
        awk '/init.espresso.usb.rc/{print "	import init.agnimounts.rc"} {print}' /tmp/extracted/init.espresso.rc > /tmp/extracted/init.espresso.rc-temp
	mv /tmp/extracted/init.espresso.rc-temp /tmp/extracted/init.espresso.rc
        awk '/mount_all/{print "	   mount_all /fstab.espresso.additional"} {print}' /tmp/extracted/init.espresso.rc > /tmp/extracted/init.espresso.rc-temp
	mv /tmp/extracted/init.espresso.rc-temp /tmp/extracted/init.espresso.rc
	sed '/FACTORYFS/d' /tmp/extracted/fstab.espresso > /tmp/extracted/fstab.espresso-temp
	mv /tmp/extracted/fstab.espresso-temp /tmp/extracted/fstab.espresso
	sed '/DATAFS/d' /tmp/extracted/fstab.espresso > /tmp/extracted/fstab.espresso-temp
	mv /tmp/extracted/fstab.espresso-temp /tmp/extracted/fstab.espresso
	sed '/CACHE/d' /tmp/extracted/fstab.espresso > /tmp/extracted/fstab.espresso-temp
	mv /tmp/extracted/fstab.espresso-temp /tmp/extracted/fstab.espresso
	sed '/HIDDEN/d' /tmp/extracted/fstab.espresso > /tmp/extracted/fstab.espresso-temp
	mv /tmp/extracted/fstab.espresso-temp /tmp/extracted/fstab.espresso
	echo " Applied modification 1 to init.espresso.rc ! " >> $AGNI_LOG_FILE
fi
if ! [ "`grep f2fs /tmp/extracted/init.espresso.rc`" ];
	then
	awk '/FACTORYFS/{print "    mount f2fs /dev/block/platform/omap/omap_hsmmc.1/by-name/FACTORYFS /system ro wait noatime nodiratime nosuid nodev discard inline_xattr active_logs=2"} {print}' /tmp/extracted/init.espresso.rc > /tmp/extracted/init.espresso.rc-temp
	mv /tmp/extracted/init.espresso.rc-temp /tmp/extracted/init.espresso.rc
	echo " Applied modification 2 to init.espresso.rc ! " >> $AGNI_LOG_FILE
fi
if ! [ "`grep AGNi-Set-SELINUX-PERMISSIVE /tmp/extracted/init.espresso.rc`" ];
	then
	echo "`cat /tmp/workboot/init.espresso-append`" >> /tmp/extracted/init.espresso.rc
	echo " Applied modification to init.espresso.rc ! " >> $AGNI_LOG_FILE
fi
...
mv /tmp/bootimg/P311x/zImage /tmp/bootimg/new-zImage && P31XX_TYPE="P311x" && touch /tmp/bootimg/zImage-is-P311x
...
echo "		P31xx type	: $P31XX_TYPE " >> $AGNI_LOG_FILE
