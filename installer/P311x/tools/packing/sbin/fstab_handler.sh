...
###### AGNi FSTAB-FS_CHECKER HANDLER v1.2 (P31xx)################
...
if [ -f $DATA_F2FS_INDICATOR ];
	then
	$BBOX mv -f /res/etc/fstab.espresso.f2fs /fstab.espresso.additional
	chmod 644 /fstab.espresso.additional
	rm $DATA_F2FS_INDICATOR
	else
	$BBOX mv -f /res/etc/fstab.espresso.ext4 /fstab.espresso.additional
	chmod 644 /fstab.espresso.additional
fi
